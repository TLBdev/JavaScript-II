...
// } 
runners.forEach((e) => {
  fullNames.push(`${e.last_name}, ${e.first_name}`)
})
// console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = runners.map(e => {return e.first_name.toUpperCase()})

// console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter(e => e.shirt_size === 'L')

// console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let priceArr = runners.map(e => {return e.donation})

let ticketPriceTotal = priceArr.reduce((a,b) => a+b)
...
// Problem 1
// get a list of all big money donors so we know who to give the really fancy stuff to.
let bigMoney = runners.filter(e => e.donation > 200)
// console.log(bigMoney)
// Problem 2
//a large corp has given a 100$ donation to each runner what is the new total
let macroHardDonation = runners.map(e => {return (e.donation + 100)}) 
let macroHardMoney = macroHardDonation.reduce((a,b) => a+b)
console.log(macroHardMoney)
// Problem 3
// yale.edu has been the target of a sustained DDoS attack and is offline indefinately create an array containing all runners with a yale.edu email address so we can update contacts.
yaleRunners = runners.filter(e => e.email.split('@')[1] === 'yale.edu')
